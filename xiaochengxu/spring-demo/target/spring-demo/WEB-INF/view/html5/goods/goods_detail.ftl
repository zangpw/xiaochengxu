...
<script>
	...
	jQuery(function() {
		...
		jQuery("#show_spec").click(function(e) {
			if(jQuery("#spec_count").css("display")=="block"){
				jQuery("#img_spec").attr("src","http://imjcoder.cn/resources/style/system/front/wap/images/details_right.gif");
			...
			jQuery("#show_count").html("x"+jQuery("#count").val());
				...
				jQuery("#img_spec").attr("src","http://imjcoder.cn/resources/style/system/front/wap/images/details_right_b.gif");
	...
	function goods_spec_set(obj){
		...
		if(load){
		jQuery.post("http://imjcoder.cn/wap/load_goods_gsp.htm",{"id":"11","gsp":gsp},function(data){
...
<body class="index_bg">
<div class="phone_hd">
	<a class="back" <#if orderState??>href="${base}/m/authc/buyer/orderList?orderState=${orderState}"<#else><#if url!=''><#if url?contains("orderView")>href="${base}/${url}"<#elseif url?contains("m/index/index")>href="${base}/m/index/index"<#else>href="${url}&refereurl=${refereurl}"</#if><#else>href="javascript:history.go(-1);"</#if></#if>>
...
<div class="details_index">
	<input type="hidden" id="goodsSpecId" value=""/>
	  <div class="details_top">
		    ...
		    <section>
		      <div class="details_price" style="height:65px;">
		        <ul>
  ...
  <div class="details_box">
    ...
    <dl class="stand" style=" border-top:1px solid #eee;">
		<a href="${base}/m/goods/goodsIntroduce?goodsId=${goodsId}&isShow=1&url=${url}&refereurl=${refereurl}&orderState=${orderState}">
	    	<dt>
    ...
    <dl class="stand">
    	<a href="${base}/m/goods/goodsConsultList?goodsId=${goodsId}&url=${url}&refereurl=${refereurl}&orderState=${orderState}">
	    	<dt>
		    	<span class="fl">
    ...
    <dl class="stand">
		<a href="${base}/m/goods/goodsEvaluteList?goodsId=${goodsId}&isShow=1&url=${url}&refereurl=${refereurl}&orderState=${orderState}">
	       	<dt>
	        	<span class="fl">
...
<script>
	...
	function query_area_data(area_1){
		...
		jQuery.ajax({
					type:'POST',
		            url:'http://imjcoder.cn/wap/buyer/address_add_ajax.htm',
					data:{"aid":aid},
					success:function(data){
	...
	function trans_fee(){
	  ...
	  jQuery.ajax({type:'POST',url:'http://imjcoder.cn/trans_fee.htm',dataType:'json',data:{"city_name":city_name,"goods_id":"11"},
