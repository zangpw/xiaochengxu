...
<div class="phone_hd"><a class="back" <#if  url!=''><#if orderState!=''>href="${base}//m/authc/buyer/orderList?orderState=${orderState}"<#else>href="${url}"</#if><#else>href="javascript:history.go(-1);"</#if>><img src="${base}/res/html5/images/back.png" width="25" height="25" /></a>返回<a class="menu home" href="${base}/m/index/index"><img src="${base}/res/html5/images/home.png" width="25" height="25" /></a></div>
    	...
    	<#if order.orderGoodsList?size gt 0>
			<#list order.orderGoodsList as orderGoods>
				<div class="order_goods">
			       	<div class="dt">
				       	<a href="${base}/m/goods/goodsdetail?id=${orderGoods.goodsId}&url=orderView_${orderId}&orderState=${orderState}">
			        ...
			        <div class="dd dd_01">
				        <span class="name">
				        	<a href="${base}/m/goods/goodsdetail?id=${orderGoods.goodsId}&url=orderView_${orderId}&orderState=${orderState}" title="${orderGoods.goodsName}">${orderGoods.goodsName[0..18]}...</a>
				        </span>
...
<div class="order_foot">
    ...
    function order_cofirm(id){
		if(confirm("确定要确认收货？")){
			window.location.href="http://imjcoder.cn/wap/buyer/order_cofirm.htm?id="+id;
		}
	}
    </script>
</div> -->
