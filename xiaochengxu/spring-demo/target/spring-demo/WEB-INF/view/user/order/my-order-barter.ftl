    	...
    	<div class="right-content"> 
      		...
      		<div class="main">
				...
				<div class="wrap">
  					<div class="tabmenu">
    					<ul class="tab pngFix">
  							<li class="active"><a  href="#">申请换货</a></li>
  						</ul>
  					</div>
  					<#if orderGoods??>
						...
						<div class="wrap-shadow">
							<div class="wrap-all ncu-order-view"> 
							    ...
							    <dl>
							      <dt>&nbsp;</dt>
							      <dd style="width:320px">
							        <p class="hint">换货信息只能填写提交一次，建议与卖家沟通后认真填写。</p>
							      </dd>
							    </dl>
							    <dl>
							    	<dt>图片信息：</dt>
...
<script type="text/javascript">
	$(function(){
		...
		$("#confirm_button").click(function(){
			...
			//判断填写换货数量为0
			if(goodsNum==0){
				layer.msg("请您至少填写一个需要换货的商品数量!",{icon:2});
				return false;
			}
			if(buyerMessage==''){
			...
			var fmUrl = '${base}/user/shopBarter';
			$.ajax({
	             type: "post",
	             url: fmUrl,
				 ...
				 success:function(data) {
					if(data.success){
						layer.alert("换货提交成功",{icon:1},function(){
							location.href="${base}/user/barterList";
						});	
					}else{
						layer.alert("换货提交失败",{icon:2},function(){
							layer.closeAll();
						});		
					}
				}
