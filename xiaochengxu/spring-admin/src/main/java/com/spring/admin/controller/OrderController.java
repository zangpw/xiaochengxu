...
//								mobile);
	...
	@ResponseBody
	public JSONObject processOrder(@RequestBody Order order) {
		JSONObject jsonObj = new JSONObject();
		try {
			order.setDataFlag((short) 1);
			Integer num = orderService.processOrder(order);
			if (num > 0) {
				jsonObj.put("result", 0);
				jsonObj.put("msg", "修改成功");
				jsonObj.put("data", num);
	...
	@RequestMapping(value="getPageWhere",method=RequestMethod.GET)
	@ResponseBody
	public JSONObject getPageByState(String states,Long productId,String trueName,Integer page, Integer rows) {
		JSONObject jsonObj = new JSONObject();
		try {
			Long memberId = null;
			List<Short> stateList = null;
			if(states!=null) {
				String[] str = states.split(",");
				stateList=new ArrayList<Short>();
				for(int i=0;i<str.length;i++) {
					stateList.add(Short.valueOf(str[i]));
				}
			}
			PageInfo<OrderVo> orderPage = orderService.getPageByState(page, rows, stateList, memberId, productId, trueName);
			jsonObj.put("result", 0);
			jsonObj.put("msg", "查询成功");
			jsonObj.put("data", orderPage);
		} catch (Exception e) {
	...
	@ResponseBody
	public JSONObject getByOrderId(Long orderId) {
		JSONObject jsonObj = new JSONObject();
		try {
			Order order = orderService.getByOrderId(orderId);
			jsonObj.put("result", 0);
			jsonObj.put("msg", "查询成功");
			jsonObj.put("data", order);
		} catch (Exception e) {
			System.err.println("出错了" + e);
			e.printStackTrace();
			jsonObj.put("result", -1);
