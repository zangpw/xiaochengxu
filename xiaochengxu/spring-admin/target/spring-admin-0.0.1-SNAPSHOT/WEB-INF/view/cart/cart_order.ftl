	...
	<input type="hidden" name="cartIds" value="${cartIds}" id="cartIds" /> 
  	<div class="content wrapper">
    		...
    		<div class="cart-title">
  			...
  			<table style="margin-top:10px; margin-left:10px; line-height:30px; height:50px;">
  				<tr>
	  				...
	  				<td  >
	  					<input type="radio" onclick=check() name="paytype" value="2"/ style="margin-left:20px;">
	  					货到付款
	  				</td>
    	...
    	<input type="hidden" name="storeId" value="${cartVo.storeId}" />
		            		...
		            		<tr id="cart_item_${cart.cartId}">
		        				<td class="w70">
		        					<div class="cart-goods-pic">
		        						<a href="${base}/product/detail?id=${cart.goodsId}" target="_blank" ><span class="thumb size60"><i></i><img src="${imgServer}${cart.goodsImages}" style="width: 60px;height: 60px;" /></span></a>
		        					</div>
		        				</td>
		        				<td class="tl vt">
      				...
      				<tr>
        				...
        				<td colspan="3" class="tr">
        					<i class="transport-ico"></i>
                   			<span id="ship_${cartVo.storeId}">免运费</span>
      	...
      	<#assign memberRankPoints = cartMap.get("member").memberRankPoints/>
      	<div class="buytable">
      		<table style="border:1px solid #A3D1EC; width:100%; margin-top:10px;  background:#eefaff">
      			<tbody style="margin-top:10px; margin-left:10px; line-height:30px; height:50px;">
      				<tr>
      					<td>
      						&nbsp;&nbsp;<input id="isPad" onclick=chageValue() type="checkbox" name="isPd" value="0"  <#if memberRankPoints==0>disabled="disabled"</#if> />
      						<label>使用积分（账户当前积分：
      							<script type="text/javascript">
	    							var pd = number_format(${memberRankPoints});
	    							document.write(pd);
	    						</script>
	    						
      						积分，本次需要：<script type="text/javascript">
	    							var totalprice = number_format(${cartMap.get('map').get('goodsTotalPrice')});
...
$(function(){
	...
	addressmember();
 	//选择已经存在的收货人地址
 	$("#defAddress").live('click',function(){
 		var id = $("#defAddress").attr("value");
    	...
    	function(){
    		if(!$(this).hasClass("selected_address")){
    			...
    			if(isDefault==0){
    				...
    				$(this).append("<a style='color:blue;' id= 'defAddress' value = '"+id+"'>设置为默认收货地址</a>");
    			}
    		}
    	},function(){
    		$(this).find("a").remove(); 
            ...
            success:function(data){
            	if(data.success){
            		...
            		$("#condition_price").html("&yen;"+toDecimal2(data.result.conditionPrice));
                	 <#--$("#predeposit_amount").html("&yen;"+toDecimal2(data.result.predepositAmount));这是之前的使用余额支付，本项目中是使用积分支付，1元=1积分-->
                	$("#memberRankPoints").html((data.result.predepositAmount));
                }
            },error:function(data){
            	layer.msg('通信失败', {icon: 2});
            }
	...
	<#--如果货到付款，那么久不用扣积分了-->
	function check(){
	var ispad=document.getElementById("isPad");
	ispad.disabled='disabled';
	}
	
	function chageValue(){
	var isPad=document.getElementById("isPad").value="1";
	alert(isPad);
