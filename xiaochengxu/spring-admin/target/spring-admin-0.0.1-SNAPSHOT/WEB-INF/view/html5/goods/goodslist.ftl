...
jQuery(document).ready(function(){
	...
	jQuery(".group_top li").click(function(){
		...
		window.location.href="${base}/m/goods/goodslist?searchType=${searchType}&keyword=${keyword}&sortField="+ searchtype + "&sortOrder=" + order +"&refereurl=${refereurl}";
	}) 	  
});<!--end-->
...
<body class="index_bg">
	<div class="phone_hd"><a class="back" <#if refereurl!=''><#if refereurl=='/m/category/category'>href="${base}${refereurl}"<#else>href="${refereurl}"</#if><#else>href="javascript:history.go(-1);"</#if>><img src="${base}/res/html5/images/back.png" width="25" height="25" /></a>商品列表<a class="menu home" href="${base}/m/index/index"><img src="${base}/res/html5/images/home.png" width="25" height="25" /></a></div>
	<div class="main">
  	<div class="mt50"></div>
  	<div class="group_top">
	    <ul>
					...
					<#if lucenePage!="" && lucenePage??>
						<#if lucenePage.result??>
							<#list lucenePage.result as goods>
							  	<#--<a href="${base}/m/goods/goodsdetail?id=${goods.goodsId}&url=referer&refereurl=/m/category/category">-->
                                <a href="${base}/m/goods/goodsdetail?id=${goods.goodsId}&url=referer&refereurl=${refereurl}">
							    <dl>
